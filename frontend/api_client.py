...
def _normalize_api_action(raw: Any) -> dict[str, Any] | None:
    ...
    return {
        ...
        "review_last_run_id": str(raw.get("review_last_run_id") or "").strip(),
        "reviewed_overlay": raw.get("reviewed_overlay", {}) if isinstance(raw.get("reviewed_overlay"), dict) else {},
        "ai_review_summary": str(raw.get("ai_review_summary") or "").strip(),
        "final_summary_instruction": str(raw.get("final_summary_instruction") or "").strip(),
        "extraction_review": raw.get("extraction_review", {}) if isinstance(raw.get("extraction_review"), dict) else {},
        "objective": str(raw.get("objective") or "").strip(),
        "input_description": str(raw.get("input_description") or "").strip(),
