...
def test_react_operational_smoke() -> None:
    with sync_playwright() as playwright:
        ...
        assistant_input = page.get_by_placeholder(
            "Digite para enviar ao campo selecionado no navegador..."
        )
        expect(assistant_input).to_be_visible(timeout=10_000)
        expect(page.get_by_role("button", name=re.compile(r"^Inserir$"))).to_be_visible()
        expect(page.get_by_role("button", name=re.compile(r"^Limpar$"))).to_be_visible()
        expect(page.get_by_role("button", name=re.compile(r"^Tab$"))).to_be_visible()
        expect(page.get_by_role("button", name=re.compile(r"^Enter$"))).to_be_visible()
        expect(page.get_by_text("Modo sensível", exact=True)).to_be_visible()
        page.get_by_role("button", name="Ocultar controles").click()
        expect(assistant_input).to_have_count(0)
        page.get_by_role("button", name="Digitar").click()
        expect(assistant_input).to_be_visible()
