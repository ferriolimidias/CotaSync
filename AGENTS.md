...
Learning must attach to the verified AccessCycle's actual CDP target and
browserContextId before enabling recording. A reattached CDP client's default
context is not proof of profile ownership. Instrument only pages belonging to
that profile; never install recorder bindings on every visible browser page.
