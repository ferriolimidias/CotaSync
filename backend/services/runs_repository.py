...
def _db_run_to_record(row: DbRun) -> RunRecord:
    ...
    record.update({
        ...
        "action_version_id": row.action_version_id or record.get("action_version_id"),
    ...
    if path is None:
        with SessionLocal.begin() as session:
            for run in runs:
                ...
                row.client_id = run.client_id or str(run.variables.get("client_id") or "") or None
                row.batch_id = run.batch_id
                row.access_profile_id = run.access_profile_id
                row.external_system_id = run.external_system_id
                row.run_start_strategy = run.run_start_strategy
