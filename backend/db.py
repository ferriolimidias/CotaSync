...
class AccessCycle(Base):
    ...
    id: Mapped[str] = mapped_column(String(128), primary_key=True)
    external_system_id: Mapped[str] = mapped_column(ForeignKey("external_systems.id", ondelete="CASCADE"), index=True)
    access_profile_id: Mapped[str | None] = mapped_column(ForeignKey("external_access_profiles.id", ondelete="SET NULL"), index=True)
    entry_url: Mapped[str] = mapped_column(Text, nullable=False)
    worker_id: Mapped[str | None] = mapped_column(String(128), index=True)
    browser_target_id: Mapped[str | None] = mapped_column(String(128))
    browser_context_id: Mapped[str | None] = mapped_column(String(128))
    manual_validation_requested_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    attention_requested_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    attention_since: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    attention_reason: Mapped[str | None] = mapped_column(String(128))
    attention_details: Mapped[dict[str, Any]] = mapped_column(JSONB, nullable=False, default=dict, server_default="{}")
