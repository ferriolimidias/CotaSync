...
function ConfigPage() {
  ...
  const removeProfile = useMutation({ mutationFn: deleteAccessProfile, onSuccess: (result) => { void queryClient.invalidateQueries({ queryKey: ["access-profiles"] }); toast.success(result.status === "retired" ? "Usuário retirado do uso operacional; histórico preservado." : "Usuário de acesso excluído definitivamente."); }, onError: (error) => toast.error(error instanceof Error ? error.message : "Não foi possível excluir o usuário de acesso.") });
                  ...
                  {(profiles.data || []).map((profile) => {
                    ...
                    <div key={profile.id} className="flex flex-col gap-2 rounded-md border border-border bg-muted/20 px-3 py-2 text-sm sm:flex-row sm:items-center sm:justify-between">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        <Button type="button" size="sm" onClick={() => { toast.message(`Iniciando acesso de ${profile.display_name}...`); void openAccessProfileWorkspace(profile.id, navigate, authenticateAccessProfile).then(() => toast.success(`Ciclo de acesso iniciado para ${profile.display_name}.`)).catch((error) => toast.error(error instanceof Error ? error.message : "Não foi possível iniciar o acesso do perfil.")); }} disabled={!loginConfigured || !profile.active}><ExternalLink className="h-4 w-4" /> Autenticar</Button>
                        <Button type="button" size="sm" variant="ghost" className="text-destructive" disabled={removeProfile.isPending || !profile.active} onClick={() => { const confirmed = window.confirm(`Excluir o usuário de acesso ${profile.display_name}? Se ele já tiver histórico, será retirado do uso operacional e o histórico será preservado. Credenciais e sessão persistente serão removidas.`); if (confirmed) removeProfile.mutate(profile.id); }}><Trash2 className="h-4 w-4" /> Excluir usuário</Button>
                      </div>
                    </div>
                    );
                  })}
