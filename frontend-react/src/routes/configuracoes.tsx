...
function ConfigPage() {
  ...
  const removeProfile = useMutation({ mutationFn: deleteAccessProfile, onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ["access-profiles"] }); void queryClient.invalidateQueries({ queryKey: ["actions"] }); toast.success("Usuário de acesso excluído; vínculos operacionais ficaram sem usuário."); }, onError: (error) => toast.error(error instanceof Error ? error.message : "Não foi possível excluir o usuário de acesso.") });
                  ...
                  {(profiles.data || []).map((profile) => {
                    ...
                    <div key={profile.id} className="flex flex-col gap-2 rounded-md border border-border bg-muted/20 px-3 py-2 text-sm sm:flex-row sm:items-center sm:justify-between">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        <Button type="button" size="sm" variant="ghost" className="text-destructive" disabled={removeProfile.isPending || !profile.active} onClick={() => { const confirmed = window.confirm(`Excluir o usuário de acesso ${profile.display_name}? Este usuário será excluído e removido das Actions/listas vinculadas. Essas automações ficarão sem usuário de acesso até você vincular outro.`); if (confirmed) removeProfile.mutate(profile.id); }}><Trash2 className="h-4 w-4" /> Excluir usuário</Button>
                      </div>
                    </div>
                    );
                  })}
